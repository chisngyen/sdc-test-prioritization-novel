"""
IMPROVEMENT 5: Focal Loss
==========================
Base: RoadTransformer (d=128, 4L, APFD=0.7899) uses BCEWithLogitsLoss.
Change: Replace with Focal Loss (Lin et al. 2017, γ=2.0).
        Down-weights easy examples, focuses on hard borderline cases.

Hypothesis: Many road tests are "obviously" pass/fail. Focal loss
            concentrates gradient on the ambiguous tests that matter.
"""
...
class FocalLoss(nn.Module):
    """FL(p) = -α (1-p)^γ log(p). Focuses on hard examples."""
...
# ★ CHANGE: Training with Focal Loss instead of BCE
def train_model(model, X_train, y_train, X_val, y_val, epochs=60, batch_size=256, lr=5e-4,
                focal_gamma=2.0, name=''):
...
def main():
    ...
    model, auc = train_model(model, X_tr, y_tr, X_te, y_te, epochs=60, focal_gamma=2.0, name='Transformer+FocalLoss')

    torch.save({'state':model.state_dict(),'means':means.tolist(),'stds':stds.tolist(),'auc':auc},
               os.path.join(OUTPUT_DIR,'imp5_focal.pt'))
    print(f"\n{'='*60}\nEVALUATION\n{'='*60}")
    print(f"\n--- SensoDat Test ({len(test_data)}) ---")
    evaluate(test_data,model,means,stds,'Transformer+FocalLoss')
    if comp_data:
        print(f"\n--- Competition ({len(comp_data)}) ---")
        evaluate(comp_data,model,means,stds,'Transformer+FocalLoss')
        print(f"\n--- Multi-trial (30) ---")
        multi_trial(comp_data,model,means,stds,'Transformer+FocalLoss')
...
============================================================
Training Transformer+FocalLoss | 828,801 params
  Focal Loss γ=2.0
============================================================
  Ep   1 | Loss:0.1836 | AUC:0.8291 | Best:0.8291 *
  Ep   2 | Loss:0.1612 | AUC:0.8556 | Best:0.8556 *
  Ep   3 | Loss:0.1503 | AUC:0.8777 | Best:0.8777 *
  Ep   5 | Loss:0.1389 | AUC:0.8942 | Best:0.8942 *
  Ep   6 | Loss:0.1299 | AUC:0.8950 | Best:0.8950 *
  Ep   7 | Loss:0.1260 | AUC:0.8992 | Best:0.8992 *
  Ep   8 | Loss:0.1213 | AUC:0.9089 | Best:0.9089 *
  Ep  10 | Loss:0.1149 | AUC:0.9125 | Best:0.9125 *
  Ep  12 | Loss:0.1068 | AUC:0.9163 | Best:0.9163 *
  Ep  15 | Loss:0.0969 | AUC:0.9146 | Best:0.9163
  Ep  16 | Loss:0.0922 | AUC:0.9171 | Best:0.9171 *
  Ep  17 | Loss:0.0901 | AUC:0.9171 | Best:0.9171 *
  Ep  18 | Loss:0.0860 | AUC:0.9183 | Best:0.9183 *
  Ep  19 | Loss:0.0827 | AUC:0.9206 | Best:0.9206 *
  Ep  20 | Loss:0.0791 | AUC:0.9146 | Best:0.9206
  Ep  25 | Loss:0.0608 | AUC:0.9168 | Best:0.9206
  Ep  30 | Loss:0.0436 | AUC:0.9105 | Best:0.9206
  Early stop ep 34
  Best AUC: 0.9206
...
--- SensoDat Test (7202) ---
  Transformer+FocalLoss                    APFD=0.7592 (7202 tests, 2765 fail)

--- Competition (956) ---
  Transformer+FocalLoss                    APFD=0.7834 (956 tests, 353 fail)

--- Multi-trial (30) ---
  Transformer+FocalLoss                    APFD=0.7820±0.0142 (30 trials)
