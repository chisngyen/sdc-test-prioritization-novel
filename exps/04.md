"""
IMPROVEMENT 4: Mixup Augmentation
===================================
Base: RoadTransformer (d=128, 4L, APFD=0.7899)
Change: Add Mixup (Zhang et al. 2018) — interpolate training pairs
        to create virtual examples and smoother decision boundaries.

Hypothesis: Mixup regularizes by training on convex combinations of
            inputs/labels, reducing overconfidence and overfitting.
"""
...
def train_model(model, X_train, y_train, X_val, y_val, epochs=60, batch_size=256, lr=5e-4,
    ...
    train_dl=DataLoader(TensorDataset(X_t,y_t),batch_size=batch_size,sampler=sampler,num_workers=2,pin_memory=True,drop_last=True)
    ...
    for epoch in range(epochs):
        ...
        for xb,yb in train_dl:
            ...
            # ★ CHANGE: Apply mixup after warmup
            if epoch >= warmup:
                xb, ya, yb_m, lam = mixup_data(xb, yb, mixup_alpha)
            else:
                ya, yb_m, lam = yb, yb, 1.0
...
def main():
    ...
    model, auc = train_model(model, X_tr, y_tr, X_te, y_te, epochs=60, mixup_alpha=0.4, name='Transformer+Mixup')

    torch.save({'state':model.state_dict(),'means':means.tolist(),'stds':stds.tolist(),'auc':auc},
               os.path.join(OUTPUT_DIR,'imp4_mixup.pt'))
    print(f"\n{'='*60}\nEVALUATION\n{'='*60}")
    print(f"\n--- SensoDat Test ({len(test_data)}) ---")
    evaluate(test_data,model,means,stds,'Transformer+Mixup')
    if comp_data:
        print(f"\n--- Competition ({len(comp_data)}) ---")
        evaluate(comp_data,model,means,stds,'Transformer+Mixup')
        print(f"\n--- Multi-trial (30) ---")
        multi_trial(comp_data,model,means,stds,'Transformer+Mixup')
...
============================================================
Training Transformer+Mixup | 828,801 params
  Mixup alpha=0.4
============================================================
  Ep   1 | Loss:0.7306 | AUC:0.8206 | Best:0.8206 *
  Ep   2 | Loss:0.6366 | AUC:0.8475 | Best:0.8475 *
  Ep   3 | Loss:0.6097 | AUC:0.8648 | Best:0.8648 *
  Ep   5 | Loss:0.5507 | AUC:0.8863 | Best:0.8863 *
  Ep   7 | Loss:0.6240 | AUC:0.8958 | Best:0.8958 *
  Ep   9 | Loss:0.6128 | AUC:0.9037 | Best:0.9037 *
  Ep  10 | Loss:0.6185 | AUC:0.9024 | Best:0.9037
  Ep  12 | Loss:0.6076 | AUC:0.9083 | Best:0.9083 *
  Ep  14 | Loss:0.5926 | AUC:0.9106 | Best:0.9106 *
  Ep  15 | Loss:0.5843 | AUC:0.9145 | Best:0.9145 *
  Ep  17 | Loss:0.5767 | AUC:0.9161 | Best:0.9161 *
  Ep  20 | Loss:0.5744 | AUC:0.9131 | Best:0.9161
  Ep  23 | Loss:0.5448 | AUC:0.9183 | Best:0.9183 *
  Ep  25 | Loss:0.5619 | AUC:0.9114 | Best:0.9183
  Ep  30 | Loss:0.5200 | AUC:0.9072 | Best:0.9183
  Ep  35 | Loss:0.4657 | AUC:0.9054 | Best:0.9183
  Early stop ep 38
  Best AUC: 0.9183
...
--- SensoDat Test (7202) ---
  Transformer+Mixup                        APFD=0.7577 (7202 tests, 2765 fail)

--- Competition (956) ---
  Transformer+Mixup                        APFD=0.7733 (956 tests, 353 fail)

--- Multi-trial (30) ---
  Transformer+Mixup                        APFD=0.7733±0.0146 (30 trials)
