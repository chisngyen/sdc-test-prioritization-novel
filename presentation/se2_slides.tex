...
\large\textcolor{navydeep}{SE(2)-Equivariant RoadNet:\\Geometric Test Prioritization for SDC}
...
\scriptsize
Ưu tiên kiểm thử cho xe tự lái bằng \textbf{bất biến hình học SE(2)}: một Transformer duy nhất, bất biến phép xoay, thắng hoặc hòa trên 8 benchmarks công khai.
\end{block}
...
\begin{itemize}
    \item Kiểm thử ngoài đời thật đắt, chậm và khó tái lập.
    \item Mô phỏng cho phép chạy hàng ngàn road scenarios có kiểm soát.
    \item Nhưng regression trên toàn bộ test suite vẫn rất tốn kém.
\end{itemize}
\end{block}

\begin{exampleblock}{Bài toán của competition}
\scriptsize
Với một test suite SDC lớn, mục tiêu là \textbf{selection / prioritization / minimization}: chọn và xếp thứ tự các kịch bản sao cho lỗi được phát hiện càng sớm càng tốt.
\end{exampleblock}

\column{0.42\textwidth}
\centering
\safeincludegraphics[width=\linewidth, keepaspectratio]{fig_hero_sdc.png}{Gemini: xe tu lai tren duong cong}\\[0.3em]
\scriptsize\textit{Trong mỗi kịch bản, xe phải giữ làn trên một hình học đường cụ thể; FAIL xảy ra khi xe lệch khỏi làn.}
\end{columns}
\end{frame}
...
\small
\begin{itemize}
    \item Nếu xếp hạng tốt $\Rightarrow$ phát hiện lỗi sớm $\Rightarrow$ \textbf{cắt 50--80\%} thời gian regression.
    \item Nếu xếp hạng kém $\Rightarrow$ lỗi nằm ở cuối hàng đợi $\Rightarrow$ chi phí gần như không giảm.
\end{itemize}
\end{block}
...
\small
\begin{itemize}
    \item Chỉ có \textbf{hình học đường}, không có hành vi xe.
    \item Phải chạy được \emph{trước} khi mô phỏng (không có nhãn thực tế).
\end{itemize}
\end{alertblock}
\end{columns}
\end{frame}
...
\scriptsize
\begin{itemize}
    \item Mọi kịch bản resample về $N = 197$ điểm.
...
\scriptsize
\textbf{Mean $|\kappa|$ của FAIL $\approx$ PASS}
($0.0284$ vs $0.0282$).\\[3pt]
$\Rightarrow$ Độ cong \textbf{trung bình} không phân biệt được FAIL/PASS. Mô hình phải đọc \emph{cấu trúc cục bộ} của $\kappa(s)$, không phải thống kê toàn cục.
\end{alertblock}
\end{columns}
\end{frame}
