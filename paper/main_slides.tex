...
\institute{University of Science, VNU-HCM \\[3pt]
           \footnotesize \textit{ICST 2026 Tool Competition -- Self-Driving Car Testing}}
\date{}
...
\begin{frame}
\centering
\LARGE \textbf{ICST 2026 Tool Competition}\\[0.5em]
...
\begin{itemize}
  ...
  \item Mất hàng nghìn giờ mỗi đợt phát hành
  \item Kỹ sư muốn thất bại xuất hiện \textbf{sớm nhất}
\end{itemize}
\end{block}
...
\scriptsize
\begin{itemize}
  \item \textbf{ICST 2026} Tool Competition: Self-Driving Car Testing
  \item \url{https://conf.researchr.org/track/icst-2026/icst-2026-tool-competition--self-driving-car-testing}
  \item 956 tests, 287-test sub-trials, 30 trials
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
\begin{frame}{Pipeline huấn luyện SE2RoadNet}
\begin{columns}[T]
\column{0.52\textwidth}
\begin{block}{Data Flow}
\scriptsize
\begin{enumerate}
  \item \textbf{Road Trajectory} $r = \{(x_i, y_i, \theta_i)\}_{i=1}^{N}$ được normalize về origin
  \item Tính curvature $\kappa(s)$ theo arc-length parameterization
  \item Trích xuất 7 kênh SE(2)-bất biến: $\{\kappa, |\kappa|, d\kappa/ds, \Delta s, \Delta\theta, \sum|\kappa|, \kappa_{\text{smooth}}\}$
  \item SE2RoadNet encoder với relative-arclength attention
  \item Listwise ranking head $\Rightarrow$ scores $s_i$
  \item ListMLE loss: $\mathcal{L} = -\sum_{i=1}^{n-1} \log \frac{\exp(s_{\pi(i)})}{\sum_{j=i}^{n} \exp(s_{\pi(j)})}$
\end{enumerate}
\end{block}
...
\begin{frame}{Tại sao dùng ListMLE loss?}
\begin{block}{Lý do chọn Listwise}
\scriptsize
\begin{itemize}
  \item \textbf{Pointwise} (MSE, BCE): dự đoán score tuyệt đối $\Rightarrow$ không tối ưu cho ranking
  \item \textbf{Pairwise} (RankNet): so sánh từng cặp $\Rightarrow$ $O(n^2)$ pairs, chậm
  \item \textbf{Listwise} (ListMLE): tối ưu trực tiếp trên permutation $\Rightarrow$ $O(n)$, gradient sparse
\end{itemize}
\end{block}

\begin{block}{ListMLE formulation}
\scriptsize
Với ground-truth ranking $\pi^*$ và predicted scores $s$:
\[
\mathcal{L}_{\text{ListMLE}} = -\sum_{i=1}^{n-1} \log P_{\theta}\big(\pi^*(i) \mid \pi^*(1..i-1)\big)
= -\sum_{i=1}^{n-1} \log \frac{\exp(s_{\pi^*(i)})}{\sum_{j=i}^{n} \exp(s_{\pi^*(j)})}.
\]
APFD gradient được ước lượng implicitly -- không cần gọi APFD trong forward pass.
\end{block}

\begin{block}{Tại sao SE(2)-invariance giúp ListMLE?}
\scriptsize
\begin{itemize}
  \item Feature extraction $\Phi$ giữ nguyên dưới SE(2) $\Rightarrow$ gradient ổn định
  \item Không cần data augmentation (không tăng training time)
  \item Invariance là \textbf{inductive bias}, không phải regularization
\end{itemize}
\end{block}
\end{frame}
...
\begin{tabular}{l l c c}
  ...
  ITEP4SDC (ICST'24) (Baseline)         & MLP, 3 aggregate stats      & 0.781             & \textcolor{alert}{$-$0.024} \\
  Greedy-diversity (TOSEM'22)            & Diversity heuristic         & 0.795             & \textcolor{alert}{$-$0.010} \\
  RoadFury                               & Transformer + SWA           & 0.804 $\pm$ 0.012 & internal \\
  \rowcolor{accent!15}
...
\begin{itemize}
  ...
  \item Tiếp tục cải thiện APFD để submit các top tier conference khác
\end{itemize}
\end{block}
\end{columns}
...
\begin{tikzpicture}[
  ...
  \node[card, text width=6cm, minimum height=2.6cm] (acc) at (-3.4, 0) {
    ...
    \textbf{bit-identical} dưới các phép xoay cứng nhắc.
  };
